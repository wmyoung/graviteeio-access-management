    ...
    public void list(
        ...
        checkAnyPermission(organizationId, environmentId, domain, application, Permission.APPLICATION_RESOURCE, Acl.LIST)
                .andThen(domainService.findById(domain)
                        ...
                        .flatMapSingle(application1 -> resourceSetService.findByDomainAndClient(domain, application1.getId(), page, Integer.min(MAX_RESOURCES_SIZE_PER_PAGE, size)))
                        .flatMap(pagedResources -> resourceSetService.getMetadata((List<ResourceSet>) pagedResources.getData())
                                .map(metadata -> new Page(Collections.singletonList(new ResourceListItem((List<ResourceSet>) pagedResources.getData(), metadata)), page, pagedResources.getTotalCount()))
                        )
                )
